...
NAME="${1:?Usage: forks/forker/replay.sh [--check] <name>}"
...
if [ "$CHECK_ONLY" -eq 0 ] && [ -d "$REAL_REPO" ]; then
  echo "ERROR: $NAME clone already exists. Use 'bash forks/forker/clean.sh $NAME' before replaying." >&2
  exit 1
fi

MANIFEST=$(manifest_file "$PIN_DIR" 2>/dev/null) || {
  echo "ERROR: $NAME has no pins to replay. Run 'bash forks/forker/record.sh $NAME' first." >&2
  exit 1
}
